...
function CheckoutContent() {
  ...
  // Support both parameter naming conventions: product_id/product (UUID primary), variant_id/variant
  ...
  const handleUploadProof = async () => {
    ...
    if (updateError) {
      ...
      // Send payment submitted notification + email via API
      try {
        await fetch('/api/payment-proof', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ order_id: createdOrder.order_id, proof_url: proofUrl }),
        })
      } catch (e) {
        console.error('Payment proof notification failed:', e)
      }
    }
