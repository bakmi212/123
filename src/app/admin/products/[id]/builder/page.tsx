...
const TEMPLATES: { name: string; blocks: BuilderBlock[] }[] = [
  {
    ...
    blocks: [
      ...
      { id: 't3', type: 'pricing', content: { title: 'Simple Pricing', price: 'Rp 79.000', period: 'one-time', features: ['Full Access', 'Lifetime Updates', 'Priority Support', 'Commercial License'], buttonText: 'Buy Now', highlighted: false, align: 'center' } },
  ...
  },
  {
    ...
    blocks: [
      ...
      { id: 'c3', type: 'pricing', content: { title: 'Enroll Today', price: 'Rp 299.000', period: 'one-time', features: ['Lifetime Access', 'All Modules', 'Community Access', 'Certificate'], buttonText: 'Enroll Now', highlighted: true, align: 'center' } },
  ...
  },
  {
    ...
    blocks: [
      ...
      { id: 's3', type: 'pricing', content: { title: 'Pricing Plans', price: 'Rp 49.000', period: '/month', features: ['Unlimited Users', 'API Access', 'Custom Integrations', 'Dedicated Support'], buttonText: 'Start Trial', highlighted: true, align: 'center' } },
  ...
  },
  {
    ...
    blocks: [
      ...
      { id: 'a4', type: 'testimonials', content: { items: [{ name: 'James Wilson', role: 'Affiliate Partner', text: 'Made my first Rp 15.000.000 in the first month.', rating: '5' }], columns: '1', align: 'center' } },
...
function LivePreviewRenderer({ blocks, product, device }: { blocks: BuilderBlock[]; product: ProductData; device: string }) {
  ...
  const getBlockActionUrl = (content: Record<string, any>): string => {
      ...
      if (navAction) {
        switch (navAction.type) {
          ...
          case 'product_purchase': {
            ...
            // Use productId from action config (UUID primary), productSlug fallback
            const productRef = productId || productSlug
            if (productRef) {
              let url = `/checkout?product_id=${productRef}`
    ...
    if (content.actionType === 'product_purchase') {
      ...
      const productRef = productId || productSlug
      if (productRef) {
        let checkoutUrl = `/checkout?product_id=${productRef}`
        if (variantId) checkoutUrl += `&variant_id=${variantId}`
        console.log('[Builder Preview] Legacy Product Purchase URL:', checkoutUrl)
        return checkoutUrl
      }
    }
    // Default product checkout (use product ID for reliable routing)
    return `/checkout?product_id=${product.id}`
  }
