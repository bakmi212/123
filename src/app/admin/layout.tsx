...
const menuItems = [
  ...
  { icon: Download, label: 'Downloads', href: '/admin/downloads', hidden: true },
  { icon: Zap, label: 'Automation', href: '/admin/automation', hidden: true },
  { icon: Users2, label: 'Affiliates', href: '/admin/affiliates' },
  { icon: Bell, label: 'Notifications', href: '/admin/notifications' },
  { icon: FileText, label: 'Blog', href: '/admin/blog' },
  { icon: BarChart3, label: 'Reports', href: '/admin/reports', hidden: true },
            ...
            {menuItems.filter((item) => !item.hidden).map((item) => (
              ...
              >
                ...
                {item.label === 'Notifications' && unreadNotifications > 0 && (
                  <Badge className="ml-auto bg-blue-600 text-white text-xs px-1.5 py-0 h-5">{unreadNotifications}</Badge>
                )}
              </Link>
            ))}
          </nav>
        ...
        <header className="sticky top-0 z-30 h-16 bg-white/80 backdrop-blur-md border-b border-slate-200">
          <div className="h-full px-4 lg:px-8 flex items-center gap-4">
            ...
            <div className="flex items-center gap-2 ml-auto">
              ...
              {/* Notifications */}
              <Link href="/admin/notifications" className="relative p-2 hover:bg-slate-100 rounded-lg transition-colors">
