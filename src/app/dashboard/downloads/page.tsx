...
export default function DownloadsPage() {
  ...
  useEffect(() => {
    const fetchDownloads = async () => {
      ...
      const { data: udRows } = await supabase
        .from('user_downloads')
        .select('id, product_id, order_id, download_count, last_downloaded_at, is_disabled, product:products(id, name, version, download_type, product_type, download_file, download_url, external_url)')
        .eq('user_id', user.id)
      ...
      if (!udRows || udRows.length === 0) {
        ...
        if (items && items.length > 0) {
          ...
          const { data: products } = await supabase
            .from('products')
            .select('id, name, version, download_type, product_type, download_file, download_url, external_url')
            .in('id', productIds)
            .or('download_type.not.is.null,product_type.eq.external_link,product_type.eq.download')
  ...
  const handleDownload = async (item: DownloadItem) => {
    ...
    let url = ''
    if (item.product_type === 'external_link' || item.download_type === 'external_url') {
      url = item.external_url || item.download_url || ''
    } else if (item.download_type === 'file_upload' && item.download_file) {
    ...
    if (!url || url.trim() === '') { toast.error('Download link not configured'); return }
              ...
              <CardContent className="p-5">
                <div className="flex items-center justify-between gap-4 flex-wrap">
                  ...
                  <Button
                    ...
                    disabled={item.is_disabled || downloading === item.product_id || (!item.download_file && !item.download_url && !item.external_url)}
                    className="shrink-0"
                  >
