...
import { Loader2, Key, Copy, CheckCircle, Clock, AlertCircle, XCircle, ShoppingBag } from 'lucide-react'
...
function calculateExpiryDate(activatedAt: string | null, licenseDuration: string | null, customDays: number | null): Date | null {
  if (!licenseDuration || licenseDuration === 'lifetime') return null
  const baseDate = activatedAt ? new Date(activatedAt) : new Date()
...
function formatDuration(duration: string | null): string {
  if (!duration || duration === 'lifetime') return 'Lifetime'
  switch (duration) {
    case '30_days': return '30 Days'
    case '90_days': return '90 Days'
    case '180_days': return '180 Days'
  ...
  useEffect(() => {
    const fetchLicenses = async () => {
      ...
      const { data } = await supabase
        .from('licenses')
        .select(`
          id, license_key, status, activated_at, expires_at, created_at, purchase_date,
          max_activations, activations_count,
          product:products(id, name, license_duration, license_type, license_limit),
      ...
      const formatted: LicenseItem[] = (data as any[])?.map(d => {
        ...
        const maxDevices = d.max_activations || product?.license_limit || 1
        return {
          id: d.id,
          ...
          license_duration: product?.license_duration || null,
          duration_type: product?.license_duration || null,
          max_devices: maxDevices,
          devices_used: d.activations_count || 0,
      ...
      {licenses.length === 0 ? (
        ...
        <div className="space-y-3">
          {licenses.map(lic => {
            ...
            // Calculate expiry based on product config if not already set
            const effectiveExpiry = lic.expires_at
              ? new Date(lic.expires_at)
              : calculateExpiryDate(lic.activated_at || lic.purchase_date, lic.license_duration, null)
            const isExpiring = effectiveExpiry && effectiveExpiry < new Date(Date.now() + 7 * 24 * 60 * 60 * 1000) && lic.status === 'active'
                ...
                <CardContent className="p-5">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap gap-4 text-xs text-slate-500">
                        <span>
                          ...
                          {new Date(lic.purchase_date || lic.created_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}
                        </span>
                        {lic.activated_at && (
                          <span>
                        ...
                        <span>
                          ...
                          {effectiveExpiry ? effectiveExpiry.toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' }) : 'Lifetime'}
                        </span>
                        <span>
                          <span className="font-medium">Devices:</span>{' '}
                          <span className="font-semibold text-slate-700">{lic.devices_used} / {lic.max_devices}</span>
                        </span>
                        {lic.order_number && (
                          <span>
                  ...
                  {isExpanded && (
                    <div className="mt-4 pt-4 border-t border-slate-100 grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                      <div><span className="text-slate-500 block text-xs font-medium mb-0.5">License Key</span>
                      ...
                      <div><span className="text-slate-500 block text-xs font-medium mb-0.5">Tanggal Pembelian</span>
                        <span>{new Date(lic.purchase_date || lic.created_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}</span></div>
                      ...
                      <div><span className="text-slate-500 block text-xs font-medium mb-0.5">Masa Aktif</span>
                        <span>{formatDuration(lic.license_duration)}</span></div>
                      <div><span className="text-slate-500 block text-xs font-medium mb-0.5">Kadaluarsa</span>
                        <span>{effectiveExpiry ? effectiveExpiry.toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' }) : 'Lifetime'}</span></div>
                      <div><span className="text-slate-500 block text-xs font-medium mb-0.5">Devices</span>
                        <span>{lic.devices_used} / {lic.max_devices} digunakan</span></div>
                      {lic.license_type && <div><span className="text-slate-500 block text-xs font-medium mb-0.5">Tipe Lisensi</span>
                        <span className="capitalize">{lic.license_type}</span></div>}
                    </div>
                  )}
                </CardContent>
              </Card>
