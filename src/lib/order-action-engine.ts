...
async function executeAction(
  ...
  try {
    switch (action.type) {
      ...
      case 'create_license': {
        const { product } = ctx
        if (!product.license_enabled && !product.enable_license) return { success: true, output: { skipped: true, reason: 'license not enabled' } }
        ...
        // Calculate expiry based on product configuration
        let expiresAt: string | null = null
        if (template?.validity_days && template.validity_days > 0) {
          const d = new Date(now); d.setDate(d.getDate() + template.validity_days); expiresAt = d.toISOString()
        } else if (product.license_duration) {
          switch (product.license_duration) {
            ...
            case '90_days': {
              const d = new Date(now); d.setDate(d.getDate() + 90); expiresAt = d.toISOString()
              break
            }
            case '180_days': {
              const d = new Date(now); d.setDate(d.getDate() + 180); expiresAt = d.toISOString()
              break
            }
            case '1_year': {
              const d = new Date(now); d.setFullYear(d.getFullYear() + 1); expiresAt = d.toISOString()
              break
            }
            case 'custom': {
              if (product.custom_license_days && product.custom_license_days > 0) {
                const d = new Date(now); d.setDate(d.getDate() + product.custom_license_days); expiresAt = d.toISOString()
              }
              break
            }
            // 'lifetime' or others = null (no expiry)
          }
        }
...
function buildDefaultActions(product: Record<string, any>): OrderActionDefinition[] {
  ...
  if (product.license_enabled || product.enable_license) {
    actions.push({
      id: 'builtin-license',
...
export async function runOrderActions(
  ...
  const { data: order, error: orderErr } = await supabase
    .from('orders')
    .select('id, user_id, affiliate_id, referral_code, click_id, referral_source, referral_url, commission_status, order_items(product_id, product:products(id, name, license_enabled, enable_license, license_duration, custom_license_days, license_limit, download_type, download_file, download_url))')
    .eq('id', orderId)
    .single()
